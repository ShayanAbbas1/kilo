...
- [x] Edit/delete sets (long-press anywhere on the set row to delete — inputs excluded, long-press there is text selection); delete past workouts — editing a *finished* workout still open
...
- [x] Stats tab: weight trend chart (90d, raw + 7-day avg lines, trend delta), sets per muscle group (7d/30d/365d/All + custom ranges, json_each over primary_muscles), calories vs target columns (14d), most-trained exercises list
...
- [x] **Body heatmap**: front+back anatomical figures on Stats colored by sets per muscle (react-native-body-highlighter), heat legend, follows the range toggle; tap a muscle to drill down (`/muscle/[slug]`: 12-week sets chart + exercises that trained it)
...
- [x] **Scientific muscle taxonomy**: audited all 873 seed exercises' primary/secondary muscles for anatomical accuracy. Dropped the non-muscle `middle back` label (rows → `lats`+`traps`, rhomboid/scapular work → `traps`) and renamed the region `lower back` → the actual muscle `erectors`; 16 canonical labels, applied everywhere (picker chips, body-map slugs, muscle-head rules). Existing installs backfilled by a v4 migration that re-applies muscle columns from the corrected seed to stock rows (custom exercises + history untouched; tested in `scripts/test-migrate-v4.mjs`).
...
- [x] **All-time + custom date ranges**: Stats muscle-group card (and its heatmap) gained an "All" chip (empty-string cutoff, matches every ISO date) and user-created Nd chips — ＋ opens a day-count input, long-press removes; saved as JSON in the `custom_ranges` settings key. All rolling windows remain `now − N days`.
...
- [x] Theme selector (Settings → Appearance): 5 hand-tuned color-scheme presets (Violet default, plus Ember, Ocean, Forest, Slate), each with light+dark variants sharing token keys so `useTheme()` stays palette-agnostic; separate Light/Dark/System mode toggle overrides the OS. Both persist in the settings table (`theme`, `theme_mode`); `Themes` registry replaces the single `Colors` object in `theme.ts`, nav chrome tracks the active palette via `_layout.tsx`'s `Nav` mounted inside `SettingsProvider`
- [ ] Custom theme (Arc-style, parked): a 6th "Custom" option = one hue slider → `tint`, with saturation/lightness locked per light/dark so every hue stays readable; structural tokens (bg/card/text/border/heatmap ramp) inherit a neutral base, `onTint` auto-derived from hue luminance. No new dep (hand-rolled slider on `react-native-svg`); `themeName === 'custom'` builds the palette at runtime from a `custom_hue` settings key. Deliberately NOT Slack-style per-slot pickers (Kilo's tint bleeds into content everywhere — no safe "chrome-only" scope to hide bad picks). Optional later: a 2nd hue for `accent`, and a shareable hue/hex code.
- [x] Ember visual identity: warm stone neutrals + ember-orange brand tint (replaces stock-iOS blue) across app + nav chrome, `onTint` token for text on orange, `accent` moved to cool cyan so Trendline kcal ≠ weight line; `Type` scale in theme.ts (stat/title/body/caption/label) applied to shared components + big stats
- [x] Space Grotesk as the app typeface (`@expo-google-fonts/space-grotesk`, runtime `useFonts` — no config plugin, so no rebuild needed): `src/components/text.tsx` Text wrapper maps `fontWeight` → per-weight font file (Android ignores numeric weights on custom fonts); all screens import Text from the wrapper; TextInputs carry explicit `fontFamily`; nav headers/tab labels themed
- [x] Premium pass: layered warm backgrounds (espresso dark / paper light — no more stock black/white), `border` token + hairline edges on cards/inputs/secondary buttons, gradient area fill under line charts + flat-bottom rounded bars, read-only workout detail redesigned as a report (Duration/Sets/Volume stat card, quiet set table) vs active workout's always-on ember "● In progress" status strip (elapsed + sets done, doubles as the rest bar)
...
- [x] Public distribution via GitHub Releases — canonical APK lives on `/releases/latest` (permanent link in README, replaces the per-build EAS artifact URL); `v1.0.0` cut with the current preview build. OTA still ships JS between releases; native changes cut a new Release. FOSS-store path (Obtainium → IzzyOnDroid) reads from these releases; F-Droid main deferred (builds from source, conflicts with OTA)
